...
tools/call) and asserts that ``list_proxy_hosts`` returns a real result. This
exercises the whole path (MCP server -> CaddyUI REST API), so the container only
reports healthy when CaddyUI is actually reachable and the token works — unlike a
bare TCP/HTTP probe. An empty CaddyUI (``[]``) still counts as healthy.
"""
    ...
    with httpx.Client(timeout=10, follow_redirects=True) as client:
        ...
        resp = client.post(
            ...
            json={
                ...
                "id": 2,
                "method": "tools/call",
                "params": {"name": "list_proxy_hosts", "arguments": {}},
            },
        )
        ...
        # CaddyUI-connectivity/auth failure as unhealthy.
        text = " ".join(
            block.get("text", "")
            for block in result.get("content", [])
            if isinstance(block, dict)
        )
        if "Error listing proxy hosts" in text:
            print(f"CaddyUI unreachable: {text}", file=sys.stderr)
            sys.exit(1)
